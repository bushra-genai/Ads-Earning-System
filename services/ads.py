...
def watch_ad(user_id):
    """
    Handle ad watching logic with daily reset.
    """
    try:
        today = date.today()
        ad_watch = AdsWatch.query.filter_by(user_id=user_id, date=today).first()
        if not ad_watch:
            ad_watch = AdsWatch(user_id=user_id, date=today, watched=False, earned_amount=0.0)
            db.session.add(ad_watch)

        if ad_watch.watched:
            raise ValueError(f"Ad already watched today for user {user_id}")

        ad_watch.watched = True
        ad_watch.earned_amount += AD_REWARD
        update_wallet_balance(user_id, AD_REWARD, 'credit')
        logging.info(f"Ad watched by user {user_id}, earned {AD_REWARD}")
        db.session.commit()
